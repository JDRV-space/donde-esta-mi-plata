...
import { AggregatedDistrictData, ImageAnalysisResult, CATEGORY_MAPPING } from '../types';
...
export const ReportFlow: React.FC<ReportFlowProps> = ({ imageFile, districts, onClose }) => {
  ...
  const [step, setStep] = useState<'analyzing' | 'captcha' | 'rejection' | 'result' | 'success' | 'municipality'>('analyzing');
  ...
  useEffect(() => {
    ...
    const runAnalysis = async () => {
      setStep('analyzing');
      try {
        const result = await analyzeImage(imageFile, language, captchaToken);
        if (cancelled) {
          return;
        }
        ...
        if (error instanceof ImageAnalysisError) {
          ...
          const seconds = error.retryAfterSeconds ?? 3600;
          alert(t(`flow.error.${error.code}`).replace('{seconds}', seconds.toString()));
        } else {
...
-----------------------------------------------
2. ${language === 'en' ? 'DETECTED PROBLEM (Photo Analysis)' : 'PROBLEMA DETECTADO (Analisis de foto)'}
-----------------------------------------------
...
`.trim();
  ...
  if (step === 'captcha') {
    return <TurnstileChallenge onSuccess={setCaptchaToken} />;
  }
  ...
  if (step === 'rejection') {
    const rejectionReason = analysis && "relevance_reason" in analysis ? analysis.relevance_reason : undefined;
      ...
      <div className="fixed inset-0 z-[2000] bg-black/90 flex flex-col items-center justify-center p-6">
        <div className="bg-retro-paper border-4 border-retro-orange shadow-retro p-8 w-full max-w-sm text-center relative">
            ...
            <p className="text-xs font-mono text-red-600 bg-red-100 p-2 border border-red-300 mb-6 italic">
                "{rejectionReason || t('flow.rejection_reason_default')}"
            </p>
            <button
                onClick={onClose}
  ...
  return (
    <div className="fixed inset-0 z-[2000] bg-retro-paper flex flex-col h-full overflow-y-auto">
      ...
      <div className="p-4 flex-1 flex flex-col gap-4 font-mono pb-24">
        ...
        {/* Card 1: Photo analysis and budget */}
        <div className="bg-white border-2 border-black shadow-retro p-4 relative">
            <div className="absolute -top-3 left-4 bg-black text-retro-amber px-2 py-1 text-xs font-bold border border-retro-amber">
                <i className="fa-solid fa-magnifying-glass mr-1"></i> {t('flow.ai_analysis')}
            </div>
